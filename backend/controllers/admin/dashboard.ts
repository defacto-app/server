   ...
   async summary(req: Request, res: Response): Promise<void> {
      ...
      try {
         ...
         const highestMenuCount = await RestaurantModel.aggregate([
            ...
            {
               $project: {
                  menuItemsCount: { $size: { $ifNull: ["$menuItems", []] } },
                  name: 1,
               },
            },
         ...
         const averageMenuCount = await RestaurantModel.aggregate([
            { $match: { menuItems: { $exists: true, $type: "array" } } }, // Ensure menuItems is an array
            {
               $project: {
                  menuItemsCount: { $size: { $ifNull: ["$menuItems", []] } },
            ...
            {
               $group: { _id: null, avgMenuItems: { $avg: "$menuItemsCount" } },
            },
         ]);
         ...
         const instantOrders = await OrderModel.countDocuments({
            isInstant: true,
         });
         ...
         console.log("dash summary error", e.message);

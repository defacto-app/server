...
async function migrateMenuCategories() {
	try {
		...
		for (let skip = 0; skip < totalMenus; skip += batchSize) {
			const menus = await MenuModel.find().skip(skip).limit(batchSize).lean();
			...
			for (const menu of menus) {
				try {
					...
					if (!isObjectId(currentCategoryId)) {
						console.log(
							`Menu "${menu.name}" categoryId is not an ObjectId: ${currentCategoryId}`,
						);
						processed++;
						continue;
					}
					...
					if (!category) {
						console.log(
							`No category found for menu: ${menu.name} (categoryId: ${currentCategoryId})`,
						);
						failed++;
						processed++;
						continue;
					...
					console.log(
						`Updated menu "${menu.name}" from ObjectId(${currentCategoryId}) to publicId(${category.publicId})`,
					);
					updated++;
				} catch (error) {
				...
				if (processed % 10 === 0) {
					console.log(
						`Progress: ${processed}/${totalMenus} (${Math.round((processed / totalMenus) * 100)}%)`,
					);
				}
			}
		}
		...
		const verificationSample = await MenuModel.find().limit(5).lean();
